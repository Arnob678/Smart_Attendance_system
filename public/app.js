  ...
  openModal(`
    ...
    <div class="form-grid">
      ...
      <div class="form-row">
        <div class="field">
          <label>Course code</label>
          <input class="input mono" id="ncCode" placeholder="e.g. ECE-2103" list="recCodeList">
          <datalist id="recCodeList"></datalist>
        </div>
        <div class="field">
          <label>Credit hours</label>
          <input class="input" type="number" id="ncCredit" value="3" step="0.25">
        </div>
      </div>
...
function onAddCourseSemesterChange(){
  const s = document.getElementById('ncSeries').value;
  const sem = Number(document.getElementById('ncSemester').value) || 1;
  const semData = RUET_ECE_ALL_SEMESTERS[sem];
  const courses = semData?.courses || [];
  ...
  if(courses.length > 0){
    recBox.style.display = 'block';
    recTitle.innerHTML = `💡 Recommended for ${s} Series — ${semData.term}:`;
    recGrid.innerHTML = courses.map(c => `
